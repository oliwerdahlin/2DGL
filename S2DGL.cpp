...
#pragma region shaders
	...
	static const char* defaultVertexShader =
		S2DGL_OPNEGL_SHADER_VERSION "\n"
		S2DGL_OPNEGL_SHADER_PRECISION "\n"
...
#pragma region Renderer2D
	...
	//won't bind any fbo
	void internalFlush(S2DGL::Renderer2D &renderer, bool clearDrawData)
	{
		enableNecessaryGLFeatures();

		if (!hasInitialized)
		{
			errorFunc("Library not initialized. Have you forgotten to call S2DGL::init() ?", userDefinedData);
		}

		if (!renderer.vao)
		{
			errorFunc("Renderer not initialized. Have you forgotten to call S2DGL::Renderer2D::create() ?", userDefinedData);
		}
	...
	void S2DGL::Renderer2D::flush(bool clearDrawData)
	{
	...
	void S2DGL::Renderer2D::renderRectangleAbsRotation(const Rect transforms, 
	...
	int  Renderer2D::wrap(const std::string &in, S2DGL::Font &f,
	...
	glm::vec2 Renderer2D::getTextSizeWrapped(const std::string &text,
		S2DGL::Font f, float maxTextLenght, float baseSize, float spacing, float lineSpacing)
	{
		std::string newText;
