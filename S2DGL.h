...
//set S2DGL_SIMD to 0 if it doesn't work on your platform
#ifdef _WIN32
#define S2DGL_SIMD 1
#else
#define S2DGL_SIMD 0
...
#define S2DGL_USE_OPENGL_130 false

#define S2DGL_DEFAULT_TEXTURE_LOAD_MODE_PIXELATED false
#define S2DGL_DEFAULT_TEXTURE_LOAD_MODE_USE_MIPMAPS true


//version of the shading language.
//#version 130 is minimum, #version 330 is right now so it can run on MacOS
#define S2DGL_OPNEGL_SHADER_VERSION "#version 330"
#define S2DGL_OPNEGL_SHADER_PRECISION "precision highp float;"

//this is the default capacity of the renderer
#define S2DGL_DefaultTextureCoords (glm::vec4{ 0, 1, 1, 0 })
...
namespace S2DGL
{
	...
	//Default error function, prints to the console.
	...
	//allows the user to set a custom error fallback function.
...
#define Colors_Red (S2DGL::Color4f{ 1, 0, 0, 1 })
#define Colors_Green (S2DGL::Color4f{ 0, 1, 0, 1 })
#define Colors_Blue (S2DGL::Color4f{ 0, 0, 1, 1 })
#define Colors_Black (S2DGL::Color4f{ 0, 0, 0, 1 })
#define Colors_White (S2DGL::Color4f{ 1, 1, 1, 1 })
#define Colors_Yellow (S2DGL::Color4f{ 1, 1, 0, 1 })
#define Colors_Magenta (S2DGL::Color4f{ 1, 0, 1, 1 })
#define Colors_Turqoise (S2DGL::Color4f{ 0, 1, 1, 1 })
#define Colors_Orange (S2DGL::Color4f{ 1, (float)0x7F / 255.0f, 0, 1 })
#define Colors_Purple (S2DGL::Color4f{ 101.0f / 255.0f, 29.0f / 255.0f, 173.0f / 255.0f, 1 })
#define Colors_Gray (S2DGL::Color4f{ (float)0x7F / 255.0f, (float)0x7F / 255.0f, (float)0x7F / 255.0f, 1 })
#define Colors_Transparent (S2DGL::Color4f{ 0,0,0,0 })
...
#pragma region Texture
	...
	struct Texture
	{
		...
		Texture() {};
		explicit Texture(const char* file, bool pixelated = S2DGL_DEFAULT_TEXTURE_LOAD_MODE_PIXELATED,
			bool useMipMaps = S2DGL_DEFAULT_TEXTURE_LOAD_MODE_USE_MIPMAPS)
		...
		void createFromBuffer(const char* image_data, const int width,
			const int height, bool pixelated = S2DGL_DEFAULT_TEXTURE_LOAD_MODE_PIXELATED, bool useMipMaps = S2DGL_DEFAULT_TEXTURE_LOAD_MODE_USE_MIPMAPS);
		...
		void createFromFileData(const unsigned char* image_file_data, const size_t image_file_size, 
			bool pixelated = S2DGL_DEFAULT_TEXTURE_LOAD_MODE_PIXELATED, bool useMipMaps = S2DGL_DEFAULT_TEXTURE_LOAD_MODE_USE_MIPMAPS);
...
#pragma region Renderer2d
	...
	struct Renderer2D
	{
		...
		//glm::vec2 spritePositions[S2DGL_Renderer2D_Max_Triangle_Capacity * 6];
		//glm::vec4 spriteColors[S2DGL_Renderer2D_Max_Triangle_Capacity * 6];
		//glm::vec2 texturePositions[S2DGL_Renderer2D_Max_Triangle_Capacity * 6];
		//Texture   spriteTextures[S2DGL_Renderer2D_Max_Triangle_Capacity * 6];
		...
		float determineTextRescaleFitSmaller(const std::string &str,
			S2DGL::Font &f, glm::vec4 transform, float maxSize);

		//determines the text size so that it fits in the given box,
		//the x and y components of the transform are ignored
		float determineTextRescaleFit(const std::string &str,
			S2DGL::Font &f, glm::vec4 transform);

		//returns number of lines
		//out rez is optional
		int wrap(const std::string &in, S2DGL::Font &f,
		...
		void renderTextWrapped(const std::string &text,
			S2DGL::Font f, glm::vec4 textPos, glm::vec4 color, float baseSize,
		...
		glm::vec2 getTextSizeWrapped(const std::string &text,
			S2DGL::Font f, float maxTextLenght, float baseSize, float spacing = 4, float lineSpacing = 3);

		//determines the text size so that it fits in the given box,
		//the x and y components of the transform are ignored
		float determineTextRescaleFitBigger(const std::string &str,
			S2DGL::Font &f, glm::vec4 transform, float minSize);

		void renderRectangle(const Rect transforms, const Texture texture, const Color4f colors[4], const glm::vec2 origin = {}, const float rotationDegrees = 0.f, const glm::vec4 textureCoords = S2DGL_DefaultTextureCoords);
		inline void renderRectangle(const Rect transforms, const Texture texture, const Color4f colors = {1,1,1,1}, const glm::vec2 origin = {}, const float rotationDegrees = 0, const glm::vec4 textureCoords = S2DGL_DefaultTextureCoords)
		{
		...
		void renderRectangleAbsRotation(const Rect transforms, const Texture texture, const Color4f colors[4], const glm::vec2 origin = {}, const float rotationDegrees = 0.f, const glm::vec4 textureCoords = S2DGL_DefaultTextureCoords);
		inline void renderRectangleAbsRotation(const Rect transforms, const Texture texture, const Color4f colors = {1,1,1,1}, const glm::vec2 origin = {}, const float rotationDegrees = 0.f, const glm::vec4 textureCoords = S2DGL_DefaultTextureCoords)
		{
			Color4f c[4] = { colors,colors,colors,colors };
